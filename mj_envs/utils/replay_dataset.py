...
def main(env_name, policy_path, mode, seed, num_episodes, render, camera_name, output_dir, output_name, save_paths, plot_paths, env_args):
    ...
    ################ replay relay kitchen datasets. dict_keys(['observations', 'actions', 'goals', 'init_qpos', 'init_qvel']) ################ 
    import pickle
    filename = 'kitchen_relax-v10_path.pkl'
    paths = pickle.load(open(f"/home/franka/dev/kathyz/relay-policy-learning/kitchen_demos_multitask/friday_microwave_topknob_bottomknob_slide/{filename}", 'rb'))
    replayed_paths, paths = env.replay_paths(
        original_paths=paths,
        horizon=env.spec.max_episode_steps,
        num_episodes=num_episodes,
    ...
    with open(f"/home/franka/dev/kathyz/relay-policy-learning/kitchen_demos_multitask/friday_microwave_topknob_bottomknob_slide/replayed_{filename}", 'wb') as handle:
        pickle.dump(replayed_paths, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ################################################################
